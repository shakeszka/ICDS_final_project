...
screen_width = window.winfo_screenwidth() - 200 #get the width of the screen
screen_height = window.winfo_screenheight() - 400 #get the height of the screen
window.geometry('%dx%d'%(screen_width, screen_height)) #set the size of the window
window.resizable(False, False)

# ==============================================================================
# Design of Home Page
# ==============================================================================
canvas = Canvas(window, height=screen_height, width=screen_width, bg="white")
login_img = PhotoImage(file="kot.gif") #load the image
canvas.create_image(0,0, anchor='nw', image=login_img)
canvas.pack(side='top')

# Text
Label(window, text='Meow! Welcome to ICS Chat System!',font=('Calibri',40), bg="white", fg="black").place(x=580, y= 120)
Label(window, text='Login Page🐱',font=('Calibri',36), bg="white", fg="black").place(x=40, y= 40)
Label(window, text='Already have an account? ', font=('Calibri',20), bg="lemonchiffon", fg="black").place(x=790, y= 215)
Label(window, text='Enter your user name🐈: ',font=('Calibri',20), bg="white", fg="black").place(x=790, y= 260)

inputUserName = StringVar()
inputUserName.set('')
entryUserName = Entry(window, textvariable=inputUserName, width=35,bg="white", fg='black').place(x=790, y=290)
Label(window, text='Enter your password🐈: ',font=("Calibri",20), bg="white", fg='black').place(x=790, y= 320)
inputUserPwd = StringVar()
entryUserPwd = Entry(window, textvariable=inputUserPwd, width=35,show='*',bg="white", fg='black').place(x=790, y=350)
usrName=''
Label(window, text='New User? ',font=('Calibri',20), bg="lemonchiffon", fg='black').place(x=790, y= 480)
...
def usrSignUp():
    ...
    canvas = Canvas(signUpWindow, height=screen_height, width=screen_width, bg = 'white')
    canvas.create_image(0,0, anchor='nw', image=login_img)
    canvas.pack(side='top')
    signUpWindow.geometry('%dx%d'%(screen_width,screen_height))
    signUpWindow.resizable(False, False)

    Label(signUpWindow, text='Create a new account: ',font=('Calibri',28), bg="white", fg='black').place(x=920, y= 150)
   
    Label(signUpWindow, text='Please enter your user name🐈: ',font=('Calibri',20), bg="white", fg='black').place(x=790, y= 230)
    Label(signUpWindow, text='Please enter your password🐈: ',font=("Calibri",20), bg="white", fg='black').place(x=790, y= 330)
    Label(signUpWindow, text='Please confirm your password🐈: ',font=("Calibri",20), bg="white", fg='black').place(x=790, y= 430)

    signName = StringVar()
    entrySignName = Entry(signUpWindow, textvariable=signName ,width=35,bg="white", fg='black').place(x=790, y=270)
    newPassword = StringVar()
    entryUserPwd = Entry(signUpWindow, textvariable=newPassword, width=35,show='*',bg="white", fg='black').place(x=790, y=370)
    newPasswordConfirm = StringVar()
    entryUserPwdConfirm = Entry(signUpWindow, textvariable=newPasswordConfirm, width=35,show='*',bg="white", fg='black').place(x=790, y=470)
...
def startChatting():
    ...
    chatWindow.geometry('%dx%d'%(screen_width,screen_height))

    #the background image
    canvas = Canvas(chatWindow, height=screen_height, width=screen_width, bg="white")
    ...
    messageList = Text(messageContainer, height = 30, bg = 'white', fg="black") # ---
    messageList.configure(state='disabled') 
    ...
    whoOnline = StringVar()
    Label(leftMenu, text="Here are all online users! Click to chat with:",font=('Calibri',14), bg="white", fg='black').grid(row=0, column=0)
